...
struct ArrayAggAggregateState {
    ...
    void update_union(const ColumnType& column, size_t offset, size_t count) {
        ArrayColumn& array_column = down_cast<ArrayColumn&>(column);
        auto offset_size = array_column.get_element_offset_size(offset);
        ColumnType& elements = array_column.elements_column();
        data_column.append(elements, offset_size.first, offset_size.second);
    }
...
class ArrayUnionAggAggregateFunction
        : public AggregateFunctionBatchHelper<ArrayAggAggregateState<LT>, ArrayUnionAggAggregateFunction<LT>> {
public:
    using InputColumnType = RunTimeColumnType<LT>;
    ...
    void update(FunctionContext* ctx, const Column** columns, AggDataPtr __restrict state,
        ...
        const auto& column = down_cast<const InputColumnType&>(*columns[0]);
        // TODO: update is random access, so we could not pre-reserve memory for State, which is the bottleneck
        this->data(state).update_union(column);
    }
    ...
    void merge(FunctionContext* ctx, const Column* column, AggDataPtr __restrict state, size_t row_num) const override {
        ...
        this->data(state).update_union(*element_data_column, offset_size.first,
    ...
    std::string get_name() const override { return "array_union_agg"; }
};
